...
(function(module){
  ...
  */
  ...
  Article.allAuthors = () => {
    return Article.all.map(eachArticle => {
      return eachArticle.author;
    }).reduce((arrayNames, currentName) => {
      if (!arrayNames.includes(currentName)) {
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(authorName => {
      return { wordCount: 0, name: authorName };
    }).map(authorCountObj => {
      let filteredByAuthor = Article.all.filter((article) => {
        return article.author === authorCountObj.name;
      });
      let authorWordCount = filteredByAuthor.map(oneArticle => {
        return oneArticle.body.split(' ').length;
      }).reduce((runningTotal, oneValueInArray) => {
  ...
  // ***TODO: Transform each author string into aobject with properties for
  // the author's name, as well as the total numbeof words across all articles
  // written by the specified author.
  // HINT: This .map should be setup to return aobject literal with two properties.
  // The first property should be prettstraightforward, but you will need to chain
  // some combination of filter, map, and reduce tget the value for the second
  // property.
